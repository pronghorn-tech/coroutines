...
package tech.pronghorn.coroutines.awaitable.future.multiWaiter
...
public class CoroutineFutureMultiWaiter<T>(private val onComplete: ((T) -> Unit)? = null) {
    ...
    companion object {
        ...
        public fun <C> completed(value: C): CoroutineFutureMultiWaiter<C> = CoroutineFutureMultiWaiter(value)
    }
    ...
    public fun externalPromise(): CoroutinePromiseMultiWaiter<T> {
        ...
        return ExternalPromiseMultiWaiter(worker, this)
    }

    public fun promise(): CoroutinePromiseMultiWaiter<T> {
        if (state != FutureState.INITIALIZED) {
            throw IllegalStateException("The promise can only be fetched immediately after creation, and only once, current state : $state")
        }
        state = FutureState.PROMISED
        return InternalPromiseMultiWaiter(this)
    }
